...
Param(
    ...
    [string] $ResourceGroupName = 'td-kv',
	[string] $StorageResourceGroupName = 'td-mvc-deploy',
    [switch] $UploadArtifacts,
    [string] $StorageAccountName,
    [string] $StorageContainerName = $DevResourceGroupName.ToLowerInvariant() + '-stageartifacts',
    [string] $TemplateFile = '../templates/kv-main.json',
    [string] $TemplateParametersFile = '../templates/kv-main.parameters.json',
...
else
{
	...
	if ($UploadArtifacts) {
		...
		$ArtifactFilePaths = (Get-ChildItem $ScriptStagingDirectory -Recurse -File | ForEach-Object -Process {$_.FullName})

		foreach ($SourcePath in $ArtifactFilePaths) {
			
			Set-AzureStorageBlobContent -File $SourcePath -Blob $SourcePath.Substring($sourcePath.LastIndexOf('\')+1) `
				-Container $StorageContainerName -Context $StorageAccount.Context -Force
		}
